...
- `relay/`: the on-demand low-latency VM, `frps`, the systemd-timer-based idle
  ...
  relay. It has a permanent static IP of its own, on a load balancer forwarding rule
  (unlike catcher, relay does use that setup by default), so there is no boot-time DNS
  update here and no DNS work at shutdown either.
